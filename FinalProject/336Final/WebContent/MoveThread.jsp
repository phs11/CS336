...
try{
	...
	String url = "jdbc:mysql://cs336-36.cs.rutgers.edu:3306/proj2014";
		...
		//query to get all topics thread is not already under
		
		String str = "SELECT T.topicName FROM Topic T WHERE NOT (T.topicName ='" + topicName +"')";
		...
		if(!result.next()){
			%>
				<jsp:forward page="Topics.jsp?topicName=<%=topicName%>"/>
			<%
		}
		do
		{
			...
			<a href="MoveThreadConfirm.jsp?threadID=<%=threadID%>&topicName=<%=topicName%>">Move to <%=topicName%></a>
			<br>
			<br>
	...
	{
		System.out.println("EXCEPTION IN MOVETHREAD " + e.getMessage());
	}

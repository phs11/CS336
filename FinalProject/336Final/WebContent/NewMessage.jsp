...
<h2>Create New Message:</h2>
	...
	<%
	if(request.getParameter("messageID") != null){ //reply to message
		try{
			...
			String message = "Select fromUser from Message where messageID= " + request.getParameter("messageID");
			ResultSet result = stmt.executeQuery(message);
			result.next();
			%>
			<form method="post" action="MessageCheck.jsp">
				<table>
					<tr>
						<td>To</td>
						<td><input type="text" name="toUser" value="<%=result.getString("fromUser")%>"></td>
					</tr>
					<tr>
						<td>Content</td>
				...
				<input type="submit" value="Create message"><button type="button" onclick="location.href='GoHome.jsp'">Cancel</button>
			</form>
			<%

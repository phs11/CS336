...
<%
//get postID of post to be moved
//get it's threadID
//print links to all move to all threads not already in in the same topic
...
try{
	...
	String url = "jdbc:mysql://cs336-36.cs.rutgers.edu:3306/proj2014";
		...
		String str = "SELECT T.topicName FROM Thread T WHERE T.threadID = '" + threadID + "'";
		...
		//query to get all other threads in the topic that it was not already under
		
		str = "SELECT T.threadID, T.description FROM Thread T WHERE T.topicName = '" + topicName + "' AND NOT (T.threadID ='" + threadID +"')";
		...
		do
		{
			threadID = result.getString("threadID");
			...
			<a href="MovePostConfirm.jsp?postID=<%=postID%>&threadID=<%=threadID%>&topicName=<%=topicName%>"> <%=description%></a>
			<br>
			<br>
	...
	{
		System.out.println("EXCEPTION IN MOVEPOST " + e.getMessage());
	}
